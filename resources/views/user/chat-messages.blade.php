<div class="settings-tray">
	<div class="friend-drawer no-gutters friend-drawer--grey">
    	...
    	<i class="material-icons"><img src="{{ asset('public/frontendassets/images/user-message.png') }}"/></i>
    	<i class="material-icons"><img src="{{ asset('public/frontendassets/images/message-flag.png') }}"/></i>-->
        </span>
  </div>
</div>
        ...
        @foreach($messages as $message)
            @if($message->file!='') 
                ...
                <div class="row no-gutters">
                    ...
                    <div class="chat-message-right {{ ($message->from == session('id')) ? '' : 'chat-message-left' }}">  
                              ...
                              <div class="chat-message-text px-4 py-3 md:p-4">
                                <div class="FileMessageBar-file FileMessageBar-file-pointer">
                                  <div class="file-info">
                                    ...
                                    <div class="file-info-bar"><span>{{$filesize}}</span><span class="uppercase">&nbsp;•&nbsp; {{$ext}} </span></div>
                                  </div>
                      ...
                      <div class="time-table">
                        <p>{{ date('d M y, h:i a', strtotime($message->created_at)) }}</p>
            ...
            @else 
                <div class="row no-gutters">
            		<div class="col-md-3 {{ ($message->from == session('id')) ? 'offset-md-9' : '' }}">
            		  <div class="chat-bubble chat-bubble--{{ ($message->from == session('id')) ? 'right' : 'left' }}">
            			@if($message->message!='')
                		    {{ $message->message }}
                		@endif
            		  </div>
                      <div class="timestamp">{{ date('d M y, h:i a', strtotime($message->created_at)) }}</div>
                      <div class="checkmark-sent-delivered">✓</div>
                      <div class="checkmark-read">✓</div>
            		</div>
        	    </div>
